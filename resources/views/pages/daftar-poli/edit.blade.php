        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg px-4 py-6">
                    <div class="container mx-auto p-4">
                        ...
                        <form method="POST" action="{{ route('daftar-poli.update', $data->id) }}">
                            @csrf
                            @method('PUT')
                            <div class="mb-4">
                                    ...
                                    <select
                                        ...
                                        class="w-full bg-transparent placeholder:text-slate-400 text-slate-700 text-sm border border-slate-200 rounded pl-3 pr-8 py-2 transition duration-300 ease focus:outline-none focus:border-slate-400 hover:border-slate-400 shadow-sm focus:shadow-md appearance-none cursor-pointer">
                                            @foreach ($jadwalPeriksa as $item)
                                                <option value="{{ $item->id }}" @selected($data->id_jadwal == $item->id)>{{ $item->dokter }} ({{ \Carbon\Carbon::parse($item->jam_mulai)->format('H:i') }}-{{ \Carbon\Carbon::parse($item->jam_selesai)->format('H:i') }})</option>    
                                            @endforeach
                                    </select>
